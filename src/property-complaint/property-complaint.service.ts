...
export class PropertyComplaintService {
  ...
  const complaints = await this.propertyComplaintRepo.find({
    where: { user: { id: userId } },
    relations: ['property', 'propertyComplaintPhotos', 'property.office'],
  });

  return complaints.map((complaint) => {
    const office = complaint.property?.office;
    return {
      ...complaint,
      office, // نقل المكتب ليكون حقل مستقل
      property: {
        ...complaint.property,
        office: undefined, // إزالة office من داخل property حتى لا يتكرر
      },
    };
  });
}
